...
知识库当前包含 11 个分类、88 个 P8 主题和 22 张关键架构/交互图。除系统设计与分布式基础外，还覆盖网络协议、存储引擎、在线迁移、Kubernetes 内核、身份协议、平台工程、AI 推理和技术治理等高阶追问。

每个主题包含业务不变量、机制、定量推导、故障恢复、适用边界、递进追问和 1-5 分评分锚点。每个分类另有一个 50 分钟综合实战，给出明确规模、SLO、故障注入、迁移回滚和带数字的推演参考。
...
知识版本升级只新增或更新内置分类、主题、图表和题目，不会删除已有的匿名学习进度、答案或面试历史。
